...
<?php 
$sql = "SELECT *, (SELECT COUNT(*) FROM likes WHERE postid = drawing.id) AS total_likes, (SELECT COUNT(*) FROM favorites WHERE drawingid = drawing.id) AS total_hearts FROM drawing ORDER BY (total_likes + total_hearts) DESC";	
                        ...
                        <div class="btn-group-nzro">
                            <div style="display: flex;justify-content: center;">
                                <div class="middlingoptions">
                                    ...
                                    ?>
                                    <span id="show_like<?php echo $row['id']; ?>" class="show_like_most" style="font-size: 20px; margin-right:10px;">
                                        <?php
...
.img-fit-poggers-related {
	...
	$sql = "SELECT * FROM drawing ORDER BY rand(id) DESC limit 4";	
	$result = mysqli_query($conn, $sql);
	$resultCheck = mysqli_num_rows($result);
	...
	$sql = "SELECT * FROM drawing ORDER BY rand(link) ASC limit 4";	
	$result = mysqli_query($conn, $sql);
	$resultCheck = mysqli_num_rows($result);
	...
	$sql = "SELECT * FROM drawing ORDER BY rand(name) ASC limit 4";	
	$result = mysqli_query($conn, $sql);
	$resultCheck = mysqli_num_rows($result);
...
<script>
    const priceNodes = document.querySelectorAll('.show_like_most');
    const sortedPriceNodes = [...priceNodes]
        .sort((b, a) => Number(a.textContent) - Number(b.textContent));
